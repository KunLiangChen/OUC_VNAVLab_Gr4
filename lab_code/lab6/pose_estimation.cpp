...
void evaluateRPE(const tf::Transform& gt_t_prev_frame,
  ...
  // ***************************** begin solution *****************************
  // 1. 将 TF 变换转换为 Eigen 类型
  Eigen::Vector3d t_gt, t_est;
  Eigen::Matrix3d R_gt, R_est;

  tf::vectorTFToEigen(gt_relative_pose.getOrigin(), t_gt);
  tf::vectorTFToEigen(est_relative_pose.getOrigin(), t_est);
  tf::matrixTFToEigen(gt_relative_pose.getBasis(), R_gt);
  tf::matrixTFToEigen(est_relative_pose.getBasis(), R_est);

  // 2. 平移归一化 (处理 2D-2D 的尺度不确定性)
  // 按照 README 建议，将 ground truth 和估计的平移都缩放到单位范数
  if (t_gt.norm() > 1e-6) t_gt.normalize();
  if (t_est.norm() > 1e-6) t_est.normalize();

  // 3. 计算平移误差 (L2 Norm)
  // RPE_tran = ||trans(T_ref) - trans(T_est)||_2
  double rpe_trans = (t_gt - t_est).norm();

  // 4. 计算旋转误差 (Chordal Distance / Frobenius Norm)
  // RPE_rot = ||rot(T_ref) - rot(T_est)||_F
  double rpe_rot = (R_gt - R_est).norm();

  // 5. 输出结果
  ROS_INFO("RPE Translation Error: %f", rpe_trans);
  ROS_INFO("RPE Rotation Error (Chordal): %f", rpe_rot);

  // 记录到文件 (建议根据当前运行的算法修改文件名)
  static std::ofstream log_file("rpe_metrics.csv", std::ios::app);
  if (log_file.is_open()) {
    // 写入：时间戳, 平移误差, 旋转误差
    log_file << ros::Time::now().toSec() << "," << rpe_trans << "," << rpe_rot << "\n";
  }
  ...
  case 0: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac) {
        ...
        // ***************************** begin solution *****************************
        // 1. 直接使用 OpenGV 5点法计算所有可能的本质矩阵
        // fivept_nister 返回 opengv::essentials_t (std::vector<Eigen::Matrix3d>)
        opengv::essentials_t essentials = opengv::relative_pose::fivept_nister(adapter_mono);
        cv::Mat R_cv, t_cv;
        int inliers = extractPose(essentials, pts1, pts2, camera_params_, R_cv, t_cv);

        // 4. 将 OpenCV 的 Mat 转换为 Eigen，再转换为 ROS Pose
        if (inliers > 0) {
            cv2Pose(R_cv, t_cv);
        } else {
            ROS_WARN("Pose estimation failed: No inliers found.");
        }
  ...
  case 1: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac) {
        ...
        // ***************************** begin solution *****************************
        // 1. 调用 OpenGV 的 8 点法算法 (Longuet-Higgins)
        // 注意：即使 8 点法通常只有一个解，OpenGV 也会返回 essentials_t 容器
        opengv::essential_t essential = opengv::relative_pose::eightpt(adapter_mono);
        opengv::essentials_t essentials_vec;
        essentials_vec.push_back(essential);
        // 3. 定义变量存储 extractPose 提取的最优旋转和平移
        cv::Mat Rmat, tmat;
        int inliers = extractPose(essentials_vec, pts1, pts2, camera_params_, Rmat, tmat);

        // 4. 将结果从 OpenCV 格式转换为 ROS Pose 格式
        // 提示中明确指出使用 cv2Pose(Rmat, tmat)
        if (inliers > 0) {
            relative_pose_estimate = cv2Pose(Rmat, tmat);
        } else {
            ROS_WARN("8-point algorithm failed to find valid pose (0 inliers).");
        }
  ...
  case 2: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      ...
      if (!FLAGS_use_ransac) {
        // Without RANSAC (OPTIONAL)
        // ***************************** begin solution *****************************
        // 1. 调用 OpenGV 2点法。注意：在这之前代码已经执行了 adapter_mono.setR12(rotation)
        // twopt 返回 translation_t (Eigen::Vector3d)
        // 传入 3 个参数：adapter, 是否使用旋转 (true), 索引
        std::vector<int> indices(adapter_mono.getNumberCorrespondences());
        std::iota(indices.begin(), indices.end(), 0);
        opengv::translation_t translation = opengv::relative_pose::twopt(adapter_mono, true, indices);

        // 2. 构造完整的变换矩阵 [R | t]
        // R 是前面已经从 GT 获取并设定的 rotation
        Eigen::Matrix<double, 3, 4> transformation;
        transformation.block<3, 3>(0, 0) = rotation;
        transformation.block<3, 1>(0, 3) = translation;

        // 3. 转换并更新结果
        relative_pose_estimate = eigen2Pose(transformation);
        ...
        // (TODO) With RANSAC
        // ***************************** begin solution *****************************
        // 1. 创建专门处理“仅平移”估计的 RANSAC 问题
        // 这里使用 2点法作为核心求解器
        std::shared_ptr<RansacProblemGivenRot> ransac_problem = 
            std::make_shared<RansacProblemGivenRot>(adapter_mono);

        // 2. 创建 Ransac 实例
        opengv::sac::Ransac<RansacProblemGivenRot> ransac;
        ransac.sac_model_ = ransac_problem;
        ransac.threshold_ = 0.00001; // 与 5 点法保持一致的阈值
        ransac.max_iterations_ = 100;
  ...
  case 3: {
    ...
    if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac){
        // Without RANSAC (Optional)
        // ***************************** begin solution *****************************
        opengv::transformation_t transformation = opengv::point_cloud::threept_arun(adapter_3d);
        relative_pose_estimate = eigen2Pose(transformation);
        // ***************************** end solution *****************************
      } else{
        // (TODO) With RANSAC
